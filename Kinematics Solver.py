...
def eval_expression(equation, variables, sqrt=False):
    ...
    equation = [
        (eval(
            j,
            variables) if i in indices_paren else (
            eval(
                j,
                variables,
                True) if i in indices_bracket else j)) for i,
        j in enumerate(equation)]
                    ...
                    if len(temp) != 1:
                        ...
                        if slash_index != -1:
                            ...
                            try:
                                ...
                                if caret_index == -1:
                                    conversion = conversions[temp[0:slash_index]
                                                             ][1] / conversions[temp[slash_index + 1:]][1]
                                    units.extend(
                                        [temp[0:slash_index], temp[slash_index + 1:]])
                                elif caret_index < slash_index:
                                    conversion = math.pow(conversions[temp[0:caret_index]][1], int(
                                        temp[caret_index + 1:caret_index + 2])) / conversions[temp[slash_index + 1:]][1]
                                    units.extend(
                                        [temp[0:caret_index], temp[slash_index + 1:]])
                                else:
                                    conversion = conversions[temp[0:slash_index]][1] / math.pow(
                                        conversions[temp[slash_index + 1:caret_index]][1], int(temp[caret_index + 1:caret_index + 2]))
                                    units.extend(
                                        [temp[0:slash_index], temp[slash_index + 1:caret_index]])
                                variables[i] = "{:.5f}".format(
                                    float(
                                        variables[i].replace(
                                            "~",
                                            "-")) *
                            ...
                            try:
                                ...
                                if caret_index != -1:
                                    conversion = math.pow(
                                        conversions[temp[0:caret_index]][1], int(temp[caret_index + 1:]))
    ...
    if i[1] is False:
        ...
        if slash_index == -1:
            if caret_index == -1:
                ...
                conversion = math.pow(conversions[t][1], int(
                    default_unit[caret_index + 1:]))
        else:
            if caret_index == -1:
                conversion = conversions[temp[default_unit[:slash_index]]
                                         ][1] / conversions[temp[default_unit[slash_index + 1:]]][1]
                default_unit = temp[default_unit[:slash_index]
                                    ] + "/" + temp[default_unit[slash_index + 1:]]
            elif caret_index < slash_index:
                conversion = math.pow(conversions[temp[default_unit[:caret_index]]][1], int(
                    default_unit[caret_index + 1:caret_index + 2])) / conversions[temp[default_unit[slash_index + 1:]]][1]
                default_unit = temp[default_unit[:caret_index]] + \
                    default_unit[caret_index:caret_index + 2] + "/" + temp[default_unit[slash_index + 1:]]
            else:
                conversion = conversions[temp[default_unit[:slash_index]]][1] / math.pow(
                    conversions[temp[default_unit[slash_index + 1:caret_index]]][1], int(default_unit[caret_index + 1:caret_index + 2]))
                default_unit = temp[default_unit[:slash_index]] + "/" + \
                    temp[default_unit[slash_index + 1:caret_index]] + default_unit[caret_index:]
...
while progress != 0:
    ...
    for i, j in enumerate(eqs):
        if variables[j[0]] is False:
            ...
            if variables[j[0]] is not False and expression is not False and math.fabs(float(
                    expression.replace("~", "-")) - float(variables[j[0]].replace("~", "-"))) > 0.2:
                impossible = True
                progress = 0
                break
...
if impossible is True or (variables["t"] is False or variables["t"][0] == "~"):
    ...
    for i, n in enumerate(constants.variables):
        print(n[1] +
              ": " +
              ("{:.5f}".format(float(variables[n[0]].replace("~", "-")) /
